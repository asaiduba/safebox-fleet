...
const AnalyticsDashboard = ({ onBack }) => {
    ...
    const fetchStats = async () => {
        try {
            ...
            const res = await axios.get(`${API_URL}/api/analytics/stats?userId=${user.id}&role=${user.role}`);
            setStats(res.data);
        } catch (err) {
    ...
    const fetchHistory = async (vehicleId) => {
        try {
            const res = await axios.get(`${API_URL}/api/analytics/history/${vehicleId}?range=${timeRange}`);
